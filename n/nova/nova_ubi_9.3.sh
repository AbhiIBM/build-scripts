...
python3 -m pip install psycopg2-binary  lxml psycopg2 pyparsing --ignore-installed
...
sh install.sh
rm -rf rust-1.63.0-powerpc64le-unknown-linux-gnu.tar.gz && cd ..     
...
sed -i '623,625d' `pwd`/nova/tests/unit/virt/test_virt_drivers.py
